  ...
  if (haudio->rd_enable == 1U && all_ready == 1U) {
    ...
    if (sof_count == 1U) {
		...
		int32_t audio_buf_writable_dev_from_nom_samples = audio_buf_writable_samples - AUDIO_TOTAL_BUF_SIZE/(2*6);
		 ...
		 // has 0ppm accuracy, and calculate the effective Fs frequency given by the PLLI2S N, R, I2SDIV and ODD register values.
		 // We then modify this nominal feedback frequency by the deviation from the ideal write pointer position wrt the read
		 // pointer over time.
		 // Need to multiply by at least a "PID k factor" of 256 for a deviation of 1 sample to produce a change in feedback (10.14).
		 // We also should use the minimum "PID k factor" that keeps the write-pointer to read-pointer distance out of the
		 // danger zone. This is to minimize the distortion caused by the host sampling frequency Fs oscillating about the nominal
		 // feedback value.
